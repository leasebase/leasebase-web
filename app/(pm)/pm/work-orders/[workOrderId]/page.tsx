...
export default function WorkOrderDetailPage() {
  ...
  const load = async () => {
    try {
      const res = await api.get<Combined>(`/pm/work-orders/${params.workOrderId}`);
  ...
  const addComment = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.post(`/pm/work-orders/${params.workOrderId}/comments`, { message: comment });
      setComment("");
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title={state.data?.workOrder.title || "Work order"}
        description="View and update maintenance work order details and comments."
      />
      {state.loading && <p className="text-sm text-slate-300">Loading work order…</p>}
